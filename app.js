...
function priceFromItem(item){
  const lows=[item.lowest_recorded_price, ...(item.offers||[]).map(o=>o.price)].map(Number).filter(n=>Number.isFinite(n)&&n>0);
...
async function searchOnlineTools(query){
  ...
  if(response.status===429) throw new Error('The free online lookup limit was reached. Try again later.');
  if(!response.ok) throw new Error('No matching products were found.');
  const data=await response.json();
  return (data.items||[]).map(normalizeLookupItem);
}
function renderLookupResults(items){
  const box=$('onlineLookupResults');
  if(!items.length){box.innerHTML='<div class="muted">No matching products found. Try the exact model number or barcode.</div>';return}
  box.innerHTML=items.slice(0,10).map((x,i)=>`<article class="lookup-result">
    ${x.image?`<img src="${esc(x.image)}" alt="">`:`<div class="no-img">No image</div>`}
    <div><h4>${esc(x.title)}</h4><p>${esc([x.brand,x.model].filter(Boolean).join(' • '))}</p><p>${x.barcode?`UPC/EAN: ${esc(x.barcode)}`:''}${x.price?` • ${money(x.price)}`:''}</p></div>
    <button class="primary" data-lookup-index="${i}">Add to Inventory</button>
  </article>`).join('');
  box.querySelectorAll('[data-lookup-index]').forEach(btn=>btn.onclick=()=>applyLookupItem(items[+btn.dataset.lookupIndex]));
}
async function applyLookupItem(item){
  ...
  if(item.price){$('purchasePrice').value=item.price;$('replacementValue').value=item.price}
  if(item.image){
    // Keep the online product image as a reference URL. User can still add their own proof photo.
    editingTool={...(editingTool||{}),onlineImage:item.image};
  }
}
async function runOnlineLookup(){
  const q=$('onlineLookupInput').value.trim();
  if(!q){$('onlineLookupStatus').textContent='Enter a tool name, model number, or barcode.';return}
  $('onlineLookupStatus').textContent='Searching online…';
  $('onlineLookupResults').innerHTML='';
  try{
    const items=await searchOnlineTools(q);
    window.__lastLookupItems=items;
    $('onlineLookupStatus').textContent=`Found ${items.length} result${items.length===1?'':'s'}.`;
    renderLookupResults(items);
  }catch(err){
    $('onlineLookupStatus').textContent=err.message||'Online lookup failed.';
  }
}
...
$('lookupFromAddBtn').onclick=()=>{$('toolDialog').close();switchView('settingsView');setTimeout(()=>$('onlineLookupInput').focus(),100)};
